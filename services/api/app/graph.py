...
def build_engineering_graph(
    ...
    def planning_node(state: EngineeringGraphState) -> dict:
        ...
        return {
            ...
            "planning": plan,
            "audit_events": [AuditEvent(agent="planning", action="create_plan", status="success")],
        }

    def architecture_node(state: EngineeringGraphState) -> dict:
        plan = state["planning"]
        if plan is None:
            raise ValueError("Architecture requires planning")
        artifact = architecture_provider.design(state["original_request"], plan)
        return {
            "status": RunStatus.ARCHITECTED,
            "architecture": artifact,
            "audit_events": [AuditEvent(agent="architecture", action="create_architecture", status="success")],
        }
    ...
    def engineering_node(state: EngineeringGraphState) -> dict:
        ...
        if plan is None or architecture is None:
            raise ValueError("Engineering requires upstream artifacts")
        artifact = engineering_provider.implement(state["original_request"], plan, architecture)
        return {
            "status": RunStatus.IMPLEMENTED,
            "engineering": artifact,
            "audit_events": [AuditEvent(agent="engineering", action="propose_change_set", status="success")],
        }

    def qa_node(state: EngineeringGraphState) -> dict:
        if state["planning"] is None or state["engineering"] is None:
            raise ValueError("QA requires artifacts")
    ...
    def security_node(state: EngineeringGraphState) -> dict:
        if state["architecture"] is None or state["engineering"] is None:
            raise ValueError("Security requires artifacts")
    ...
    def quality_gate_node(state: EngineeringGraphState) -> dict:
        ...
        if qa is None or security is None:
            raise ValueError("Quality Gate requires both checks")
        blocking = [item.id for item in [*qa.findings, *security.findings] if item.blocking]
        passed = qa.passed and security.passed and not blocking
        gate = QualityGateArtifact(
            passed=passed,
            blocking_findings=blocking,
            decision="continue_to_review" if passed else "return_to_engineering",
        )
    ...
    def review_node(state: EngineeringGraphState) -> dict:
        ...
        return {
            ...
            "review": review,
            "audit_events": [AuditEvent(agent="reviewer", action="review_traceability", status="success" if review.passed else "blocked")],
        }

    graph = StateGraph(EngineeringGraphState)
    for name, node in [
        ("planning", planning_node), ("architecture", architecture_node), ("engineering", engineering_node),
        ("qa", qa_node), ("security", security_node), ("quality_gate", quality_gate_node), ("review", review_node),
    ]:
    ...
    graph.add_conditional_edges("quality_gate", route_quality, {"review": "review", "end": END})
